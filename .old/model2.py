...
all_instances = read_instances(path_instances_translated)[2:]
...
for idx, instance in enumerate(all_instances[-1:]):
    ...
    for j in instance['PT']:
        for l in instance['PT'][j]:
            for i in instance['PT'][j][l]:
                if l == 0:
                    model.addConstr(startT[j][l][i] >= instance['STC'][j], name=f"initial_start_time_job{j}_stage0_machine{i}_constraint")
