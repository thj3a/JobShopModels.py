...
def translate_instance(path, instance_name):
    ...
    alter.sort_values(by=['CdItem', 'NuEstagio', 'CdMaq'])
    ...
    initial_restrictions.sort_values(by='CdItem')

    for i in initial_restrictions.itertuples():
        diff = (i.DtReceb - initial_date).total_seconds()/60
        lines.append([diff] if diff > 0 else [0.0])
