...
def read_instance(instance_name, instance_path):
    ...
    with open(path, 'r') as f:
        ...
        instance['n_jobs'] = int(lines_0[0]) # number of jobs
        instance['n_machines'] = int(lines_0[1]) # number of machines
        instance['jobs'] = [i for i in range(instance['n_jobs'])] # jobs are indexed from 0 to n_jobs-1
        instance['machines'] = [i+1 for i in range(instance['n_machines'])] # machines are indexed from 1 to n_machines
        instance['P'] = {i:dict() for i in instance['jobs']} # processing times
        instance['R'] = {i:dict() for i in instance['jobs']} # resources
        instance['Q'] = {j: 0 for j in instance['jobs']} # starting times
        instance['D'] = {j: 0 for j in instance['jobs']} # deadlines
