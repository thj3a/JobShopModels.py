...
def validate_solution(instance, df, x, y, startT, C_max):
    ...
    for m in machines:
        ...
        for machine in range(len(df_machine)-1):
            h, z, j, l, i = map(int, [df_machine.loc[machine, 'Job'], df_machine.loc[machine, 'Op'], df_machine.loc[machine+1, 'Job'], df_machine.loc[machine+1, 'Op'], df_machine.loc[machine, 'Resource'][-1]])
            if df_machine.loc[machine, 'Finish_f'] + instance['ST'][i][h][z][j][l] > df_machine.loc[machine+1, 'Start_f'] :
    ...
    for job in jobs:
        ...
        for machine in range(len(df_job)-1):
            if df_job.loc[machine, 'Finish'] > df_job.loc[machine+1, 'Start']:
...
def validate_vars(x, y, startT, instance):
    ...
    for job in instance['PT']:
        for op in instance['PT'][job]:
            if sum([y[job][op][i].x for i in instance['R'][job][op]]) != 1:
