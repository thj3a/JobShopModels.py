...
def validate_solution(instance, timestamp):
    ...
    for m in machines:
        ...
        for line in range(len(df_machine)-1):
            ...
            if round(df_machine['Finish_f'].values[line]) + instance.O[h][z][j][l][i] > round(df_machine['Start_f'].values[line+1]) :
                ...
                print(f"job1|op1: {h}|{z}: {df_machine['Finish_f'].values[line]}, setup: {instance.O[h][z][j][l][i]}, job2|op2 {j}|{l}: {df_machine['Start_f'].values[line+1]}")
                return False
