...
if __name__ == "__main__":
    ...
    running_list = list(itertools.product(all_instances, [Objective.MAKESPAN]))
    ...
    for idx, (instance, objective) in enumerate(running_list):
        ...
        # Create empty model
        model = Model(name="JobShopModel", log_output = True)
        model.parameters.randomseed = 1
        model.context.log_output = open(f"{results_name}/log/{instance_name}.log", 'w')
        model.context.verbose = 9
        ...
        if solution:
            ...
            if save_output:
                ...
                # save solution
                with open(f"{results_name}/json/{instance_name}.json", 'w') as f:
                    solution.export(file_or_filename =f"{results_name}/json/{instance_name}.json", format='json')
